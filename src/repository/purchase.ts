...
export class PurchaseRepository {
  ...
  static async createStatus(data: { purchaseID: number; status: string; createdBy: number; description?: string }) {
    return prisma.purchaseStatus.create({ data: { ...data, createdAt: new Date() } })
  }

  static async getAll(page: number, pageSize: number, filter: PurchaseFilter = {}, keyword?: string, sortBy = "createdAt", sortDir = "desc") {
    ...
    const [data, total] = await Promise.all([
      prisma.purchase.findMany({ where: where as never, orderBy: { [sortBy]: sortDir as "asc" | "desc" }, skip, take }),
  ...
  static async getByPurchaseOrderName(name: string) {
    return prisma.purchase.findMany({ where: { purchaseOrderName: name, isDelete: false } })
  }
  ...
  static async checkExists(invoiceName: string, purchaseOrderName: string) {
    return prisma.purchase.findFirst({ where: { invoiceName, purchaseOrderName, isDelete: false } })
  }
}
