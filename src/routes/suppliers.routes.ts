...
import type { ISupplierCreate, ISupplierUpdate } from "../interfaces/supplier.interfaces"
...
export const supplierRoutes = new Elysia({ prefix: "/suppliers" })
  ...
  .post("/", async ({ body, user, set }) => {
    const result = await SupplierController.create(body as ISupplierCreate, user.id)
    if ("error" in result) { set.status = (result as any).status; return result }
    return result
  }, { body: SupplierBody })
  .put("/", async ({ body, user, set }) => {
    const { id, ...rest } = body as ISupplierUpdate & { id?: number }
  ...
  .put("/:id/blacklist", async ({ params, body, user, set }) => {
    const flag = (body as any).isBlackListed
    if (typeof flag !== "boolean") { set.status = 400; return { detail: "isBlackListed boolean is required" } }
    const result = await SupplierController.setBlackList(Number(params.id), flag, user.id)
    if ("error" in result) { set.status = (result as any).status; return result }
    return result
  }, { body: t.Object({ isBlackListed: t.Boolean() }) })
