...
export const purchaseRoutes = new Elysia({ prefix: "/purchases" })
  ...
  .post(
    ...
    async ({ body, user, set }) => {
      const b = body as any;
      const existing = await PurchaseRepository.checkExists(
        b.invoiceName,
        b.purchaseOrderName,
      );
  ...
  .get("/payments/:id", async ({ params, user, set }) => {
    const payments = await prisma.paymentOutgoing.findMany({
      where: { purchaseID: Number(params.id), isDelete: false },
    });
    return { payments };
  })
  .get("/:id", async ({ params, user, set }) => {
    ...
    if (!purchase) {
      set.status = 404;
      return { detail: "Purchase not found" };
    }
    const payments = await prisma.paymentOutgoing.findMany({
      where: { purchaseID: Number(params.id), isDelete: false },
    });
  ...
  .get("/", async ({ query, user, set }) => {
    ...
    try {
      ...
      logError(`${e}`);
      set.status = 500;
      return { detail: "Failed to fetch purchases" };
    }
  })
  .post(
    "/",
    async ({ body, user, set }) => {
      try {
        ...
        await PurchaseRepository.createStatus({
        ...
        logError(`${e}`);
        set.status = 500;
        return { detail: "Failed to create purchase" };
      }
    },
    { body: PurchaseBody },
  )
  .put(
    ...
    async ({ body, user, set }) => {
      const b = body as any;
      try {
        await PurchaseRepository.update(b.purchaseID, {
          lastStatus: "ready",
          lastStatusDescription: b.description,
          updatedBy: user.id,
          updatedAt: new Date(),
        });
        await PurchaseRepository.createStatus({
        ...
        logError(`${e}`);
        set.status = 500;
        return { detail: "Failed to update status" };
      }
    },
    {
      body: t.Object({
  ...
  .delete("/:id", async ({ params, user, set }) => {
    try {
      ...
      logError(`${e}`);
      set.status = 500;
      return { detail: "Failed to delete purchase" };
    }
  });
