...
@router.get("/")
async def get_purchase_draft(page: int, pageSize: int, isPending: bool, isApproved: bool, sortBy: str, sortByDirection: str, keyword: str, current_user: Annotated[User, Depends(get_current_user)]):
...
@router.put("/")
async def update_purchase(purchase: dict, current_user: Annotated[User, Depends(get_current_user)]):
    """
...
async def delete_purchase_draft(purchaseDraftID: int, current_user: Annotated[User, Depends(get_current_user)]):
    userID = current_user["id"]
